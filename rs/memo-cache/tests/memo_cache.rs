mod tests_external {
    ...
    fn test_nonempty() {
        ...
        assert!(c.get(kv0.0.to_owned()).is_none());
        assert!(c.get(kv1.0.to_owned()).is_none());
        assert!(c.get(kv2.0.to_owned()).is_none());

        c.insert(kv0.0.to_owned(), kv0.1);

        assert_eq!(c.get(kv0.0.to_owned()), Some(&kv0.1));

        c.insert(kv1.0.to_owned(), kv1.1);

        assert_eq!(c.get(kv1.0.to_owned()), Some(&kv1.1));

        c.insert(kv2.0.to_owned(), kv2.1);

        assert_eq!(c.get(kv2.0.to_owned()), Some(&kv2.1));
        ...
        assert!(c.get(kv0.0.to_owned()).is_none());
        assert!(c.get(kv1.0.to_owned()).is_some());
        assert!(c.get(kv2.0.to_owned()).is_some());
    ...
    fn test_duplicate_insertions() {
        ...
        assert!(c.get(kv0.0.to_owned()).is_some());
        assert!(c.get(kv1.0.to_owned()).is_some());
        ...
        assert_eq!(c.get(kv0.0.to_owned()), Some(&42)); // Updated.
        assert_eq!(c.get(kv1.0.to_owned()), Some(&kv1.1));
    }
}
